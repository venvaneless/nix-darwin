...
in
{
  config = lib.mkIf (platforms.enabledForCurrentPlatform cfg) {
    programs.fish.functions = {
      ...
      # -----------------------------------------------------------------
      __obsidian_command_download_name_blocked = {
        ...
        body = ''
          ...
          set --local blocked_document_stems \
              ${renderFishList policy.exclusions.normalizedTitleStems}
      ...
      # -----------------------------------------------------------------
      __obsidian_command_download_path_blocked = {
        description = "Check whether an Obsidian download path is blocked";

        body = ''
          set --local download_path "$argv[1]"
          set --local download_path_lower (string lower -- "$download_path")

          # Content policy is enforced before a repository path is eligible.
          # Core files at the entry root remain available; these switches only
          # constrain optional documentation, media, snippets, and nesting.
          if not ${if policy.content.keepDocumentation then "true" else "false"}; and \
              string match -rq '\\.(md|markdown|org)$' "$download_path_lower"
            return 0
          end

          if not ${if policy.content.keepImages then "true" else "false"}; and \
              string match -rq '\\.(gif|jpe?g|png|webp)$' "$download_path_lower"
            return 0
          end

          if not ${if policy.content.keepReadme then "true" else "false"}; and \
              string match -rqi '(^|/)readme(?:\\.[^/]+)?$' "$download_path"
            return 0
          end

          if not ${if policy.content.keepSnippets then "true" else "false"}; and \
              string match -rqi '(^|/)snippets?/' "$download_path"
            return 0
          end

          if not ${if policy.content.keepNestedContent then "true" else "false"}; and \
              string match -q '*/*' "$download_path"
            return 0
          end

          if __obsidian_command_download_name_blocked (basename "$download_path")
            return 0
          end
          ...
          set --local locale_markers ${renderFishList policy.exclusions.localeMarkers}
          set --local hidden_paths ${renderFishList policy.exclusions.hiddenPaths}
          set --local blocked_path_segments ${renderFishList policy.exclusions.pathSegments}

          for path_part in (string split / -- "$download_path")
            set --local path_part_lower (string lower -- "$path_part")
            if contains -- "$path_part_lower" $hidden_paths; or \
                contains -- "$path_part_lower" $blocked_path_segments
              return 0
            end
      ...
      # -----------------------------------------------------------------
      __obsidian_command_normalize_download_layout = {
        description = "Normalize downloaded Obsidian documentation and image layouts";

        body = ''
          set --local library_entry "$argv[1]"

          if not ${if policy.content.normalizeLayout then "true" else "false"}
            return 0
          end
      ...
      # -----------------------------------------------------------------
      __obsidian_command_repository_fallback_name = {
      ...
      # -----------------------------------------------------------------
      __obsidian_command_is_named_release_archive = {
        ...
        body = ''
          if test (count $argv) -gt 0; and \
            ...
            if test "$mode" = "--plugins"
              ...
              if not functions -q __obsidian_command_gitdll_themes
            ...
            function __obsidian_command_gitdll_required_files_exist \
            ...
            function __obsidian_command_gitdll_remote_matches \
              ...
              __obsidian_command_gitdll_required_files_exist "$candidate" "$library_type"
            end

            function __obsidian_command_gitdll_resolve_repository \
              ...
              for direct_owner in $direct_owners
                ...
                if __obsidian_command_gitdll_required_files_exist \
                    "$direct_candidate" \
                    "$library_type"
              ...
              for candidate in $candidates
                if __obsidian_command_gitdll_remote_matches \
                    "$candidate" \
                    "$library_id" \
                    "$library_author" \
                    "$library_type"
            ...
            function __obsidian_command_gitdll_download_includes \
            ...
            while read --local source_mapping
              ...
              if test -z "$matching_manifest_file"
                ...
                __obsidian_command_gitdll_download_includes \
                  "$original_url" \
            ...
            functions -e __obsidian_command_gitdll_write_failure_report
            functions -e __obsidian_command_gitdll_download_includes
      ...
      # -----------------------------------------------------------------
      # ---- __obsidian_command_gitdll_plugins -> Internal Obsidian plugin downloader ---- #
      # -----------------------------------------------------------------
      __obsidian_command_gitdll_plugins = {
        ...
        body = ''
          ...
          function __obsidian_command_gitdll_plugin_stylesheet_is_healthy --argument-names stylesheet_file
          ...
          function __obsidian_command_gitdll_plugin_core_is_healthy --argument-names plugin_directory
          ...
          function __obsidian_command_gitdll_plugin_payload_is_healthy --argument-names plugin_directory
              if not test -r "$plugin_directory/manifest.json"; or \
                  not test -s "$plugin_directory/manifest.json"; or \
                  not command jq -e 'type == "object"' "$plugin_directory/manifest.json" >/dev/null 2>&1; or \
                  not test -r "$plugin_directory/main.js"; or \
          ...
          function __obsidian_command_gitdll_replace_plugin_core_from_url --argument-names destination_file download_url
          ...
          function __obsidian_command_gitdll_replace_plugin_core_from_file --argument-names source_file destination_file
              set --local staging_file "$destination_file.gitdll-new"
              command rm -f -- "$staging_file"
          ...
          for repository_url in $repositories
              ...
              if test $status -eq 0; and test -n "$release_json"
                  ...
                  for release_asset in $release_assets
                      ...
                      if __obsidian_command_download_path_blocked \
                              "$release_asset_name"
              ...
              for expected_file in $plugin_required_files $plugin_optional_files
              ...
              if test -z "$manifest"
                  ...
                  set generated_plugin_id (
                      __obsidian_command_repository_fallback_name "$repository_name"
                  )
              ...
              if test -d "$plugin_directory"
                  ...
                  if __obsidian_command_gitdll_plugin_core_is_healthy "$plugin_directory"; and \
                          command jq -e \
              ...
              if not test -r "$plugin_stage/manifest.json"; or \
                      ...
                      not command jq -e . \
                          "$plugin_stage/manifest.json" \
                          >/dev/null 2>&1
                  __obsidian_command_gitdll_replace_plugin_core_from_file \
                      "$manifest" \
                      "$plugin_stage/manifest.json"
              end
                  ...
                  for release_asset in $release_assets
                      ...
                      switch "$release_asset_name"
                          case main.js data.json styles.css manifest.json
                              ...
                              if __obsidian_command_gitdll_replace_plugin_core_from_url \
                          ...
                          case '*'
                              if __obsidian_command_is_named_release_archive \
                                      "$release_asset_name" \
                                      "$plugin_folder_name" \
                                      "$repository_name"
              ...
              if not __obsidian_command_gitdll_plugin_payload_is_healthy "$plugin_stage"
              ...
              __obsidian_command_normalize_download_layout "$plugin_stage"
      ...
      # -----------------------------------------------------------------
      # ---- __obsidian_command_gitdll_themes -> Internal Obsidian theme downloader ---- #
      # -----------------------------------------------------------------
      __obsidian_command_gitdll_themes = {
        ...
        body = ''
          ...
          function __obsidian_command_gitdll_theme_stylesheet_is_healthy --argument-names stylesheet_file
              if not test -f "$stylesheet_file"; or not test -s "$stylesheet_file"
                  return 1
              end
              command node -e '
          ...
          function __obsidian_command_gitdll_replace_theme_core_from_file --argument-names source_file destination_file
              set --local staging_file "$destination_file.gitdll-new"
              command rm -f -- "$staging_file"
              if command cp -- "$source_file" "$staging_file"; and test -s "$staging_file"
                  command mv -- "$staging_file" "$destination_file"
          ...
          for repository_url in $repositories
              ...
              if test $status -eq 0; and test -n "$release_json"
                  ...
                  for release_asset in $release_assets
                      ...
                      if __obsidian_command_download_name_blocked \
                              "$release_asset_name"
              ...
              if test -d "$theme_directory"
                  ...
                  if test "$theme_manifest_ok" -eq 0; or \
                      not __obsidian_command_gitdll_theme_stylesheet_is_healthy "$theme_directory/theme.css"
              ...
              if test -n "$manifest"; and test -f "$manifest"
                  if not test -r "$theme_stage/manifest.json"; or \
                      ...
                      __obsidian_command_gitdll_replace_theme_core_from_file \
                          "$manifest" \
                          "$theme_stage/manifest.json"
                  end
              ...
              __obsidian_command_normalize_download_layout "$theme_stage"
      ...
      __obsidian_command_missing = {
        ...
        body = ''
          ...
          function __obsidian_command_missing_canonical_repository_url --argument-names repository_url
          ...
          function __obsidian_command_missing_manifest_is_healthy --argument-names manifest_file
          ...
          function __obsidian_command_missing_javascript_is_healthy --argument-names script_file
          ...
          function __obsidian_command_missing_stylesheet_is_healthy --argument-names stylesheet_file
          ...
          function __obsidian_command_missing_release_asset_url \
          ...
          function __obsidian_command_missing_download_url \
          ...
          function __obsidian_command_missing_path_is_unsupported --argument-names repository_path
            if not string match -rq '^[\x00-\x7F]+$' "$repository_path"
              return 0
            end

            if __obsidian_command_download_path_blocked "$repository_path"
              return 0
            end
          ...
          function __obsidian_command_missing_restore_readme \
            ...
            if __obsidian_command_missing_path_is_unsupported "$readme_name"
              return 0
            end
          ...
          # as gitdll --themes.
          function __obsidian_command_missing_restore_theme_screenshots \
              --argument-names library_entry repository_url
            ...
            for repository_path in $repository_paths
              ...
              if string match -rq \
                  ...
                  '(^|/)node_modules/' \
                  "$repository_path"; or \
                  __obsidian_command_missing_path_is_unsupported \
                      "$repository_path"; or \
                  __obsidian_command_download_name_blocked \
                      "$repository_image_name"
          ...
          function __obsidian_command_missing_restore_standard_files \
            ...
            if test "$requires_plugin_payload" -eq 1
              ...
              if test -e "$library_entry/styles.css"; and \
                  not __obsidian_command_missing_stylesheet_is_healthy \
                    "$library_entry/styles.css"
                set plugin_stylesheet_is_healthy 0
              end

              if not __obsidian_command_missing_manifest_is_healthy \
                  "$library_entry/manifest.json"; or \
                  not __obsidian_command_missing_javascript_is_healthy \
                    "$library_entry/main.js"; or \
                  test "$plugin_stylesheet_is_healthy" -eq 0
                set refresh_core_payload 1
              end
            else if not __obsidian_command_missing_manifest_is_healthy \
                "$library_entry/manifest.json"; or \
                not __obsidian_command_missing_stylesheet_is_healthy \
            ...
            for release_asset in $release_assets
              ...
              set --local release_asset_url \
                ...
                if __obsidian_command_missing_path_is_unsupported \
            ...
            if test "$requires_plugin_payload" -eq 1
              ...
              for expected_file in manifest.json main.js styles.css data.json
                ...
                if test -n "$release_url"
                  if __obsidian_command_missing_download_url \
                      "$destination" \
                      "$release_url" \
                      "$expected_file" \
                      yes
            ...
            if test "$requires_plugin_payload" -eq 1
              if __obsidian_command_missing_manifest_is_healthy \
                  "$library_entry/manifest.json"; and \
                  __obsidian_command_missing_javascript_is_healthy \
                    "$library_entry/main.js"
                set library_core_available 1
              end
            else if __obsidian_command_missing_manifest_is_healthy \
                "$library_entry/manifest.json"; and \
                __obsidian_command_missing_stylesheet_is_healthy \
                  "$library_entry/theme.css"
              set library_core_available 1
            end
            ...
            __obsidian_command_missing_restore_readme \
              "$library_entry" \
              "$repository_url"
            ...
            if not __obsidian_command_missing_manifest_is_healthy "$library_entry/manifest.json"
            ...
            __obsidian_command_normalize_download_layout "$library_entry"
          ...
          set --local missing_report "${commandPaths.missingReport}"
          ...
          for library_entry in $library_entries
            ...
            if test -n "$manifest_repository_url"
              set manifest_repository_url (__obsidian_command_missing_canonical_repository_url "$manifest_repository_url")
            end
            ...
            if test -n "$manifest_repository_url"
              ...
              if test -n "$current_repository_url"; and \
                  not __obsidian_command_missing_manifest_is_healthy "$manifest_file"
            ...
            for direct_owner in $direct_owners
              ...
              set --local direct_release_manifest_url (
                __obsidian_command_missing_release_asset_url \
            ...
            for candidate in $candidates
              set --local candidate_release_manifest_url (
                __obsidian_command_missing_release_asset_url "$candidate" manifest.json
              )
              ...
              if test "$requires_plugin_payload" -eq 1
                set --local candidate_main_url (
                  __obsidian_command_missing_release_asset_url "$candidate" main.js
                )
