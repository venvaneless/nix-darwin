...
let
  ...
  # Home Manager's programs.wezterm module always installs its package in the
  # user profile. Render the same Lua configuration directly so the system
  # package declared in shared/packages.nix remains WezTerm's only install.
  weztermSettings = cfg.ssh // {
  ...
  weztermConfig = ''
    -- Generated by nix-config's WezTerm option module.

    local wezterm = require 'wezterm'
    local config = wezterm.config_builder and wezterm.config_builder() or {}
    local nix_config = ${lib.generators.toLua { multiline = true; } weztermSettings}

    for key, value in pairs(nix_config) do
      config[key] = value
    end
...
in
{
  ...
  config = lib.mkIf (cfg.enable && platforms.enabledForCurrentPlatform cfg) {
    ...
    # Rendered directly rather than through programs.wezterm, which would
    # install a second user-profile WezTerm application.
