...
let
  # The entry point is selected before the Nix module system invokes its
  # function. This keeps helper-only arguments out of the module function
  # while all package logic remains in this option module.
  mkPackageHelpers =
    {
      lib,
      paths,
    ...
    let
        ...
        packageInstallTarget = installTarget;
...
if mode == "helper" then
  ...
  mkPackageHelpers {
    inherit lib paths platforms pkgs symlinks installTarget;
  }

else if mode == "module" then
  {
    config,
    lib,
    paths,
  ...
  let
    packageHelpers = mkPackageHelpers {
      inherit lib paths platforms pkgs symlinks;
      installTarget = "system";
    };
    inherit (packageHelpers) mkPackageModule;
      # ------------------------------------------------------------
