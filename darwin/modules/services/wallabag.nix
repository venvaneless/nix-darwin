...
in
{
  ...
  options.services.wallabag = {
    ...
    # Define the type of the option as a string
      ...
      default = "/Users/ven/Library/Mobile Documents/com~apple~CloudDocs/my-system/user-data/databases-containers/wallabag";
  ...
  config = lib.mkIf cfg.enable {
    ...
    system.activationScripts.ensureWallabagDataDir.text = lib.mkAfter ''
      ...
      /usr/bin/install -d -m 0775 -o ven -g staff "${cfg.dataDir}"

      # Wallabag's persistent data storage directory
      /usr/bin/install -d -m 0775 -o ven -g staff "${cfg.dataDir}/data"

      # Wallabag's images storage directory
      /usr/bin/install -d -m 0775 -o ven -g staff "${cfg.dataDir}/images"
    '';
      ...
      # ** 'ven:staff': Specify the new owner (user 'ven') and group ('staff') for the directory and its contents
