# shared/terminal/cli-tuis/micro/themes/default-theme.nix
#
# =====================================================================
# MICRO: DEFAULT THEME
#
# - Default theme for Micro
# - Selected directly in ../micro.nix with selectedTheme = "default"
# =====================================================================
...
let
  ...
  cfg = microCfg.themes.default;
in
{
  options.ven.features.terminal.cliTuis.micro.themes.default.enable = lib.mkOption {
    type = lib.types.bool;
    default = false;
    description = "Internal switch for the Default theme selected in micro.nix.";
  };

  config = lib.mkIf (microCfg.enable && cfg.enable) {
    # Micro loads custom colour schemes from this XDG colourschemes directory.
    xdg.configFile."micro/colorschemes/default.micro".text = ''
